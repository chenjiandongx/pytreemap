...
def get_parser():
    ...
    parser = argparse.ArgumentParser(
        description="树图渲染命令行工具-利用 JSON 数据生成 HTML 格式的树图", add_help=False
    )
    parser.add_argument("-i", "--input", type=str, help="JSON 数据路径.")
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        default="TreeMap.html",
    ...
    parser.add_argument(
        ...
        type=str,
        default="LR",
        help="树图的布局方向, " "有 LR/RL/H/TB/BT/V 可选.(默认为 LR)",
    )
    parser.add_argument(
        "-t",
        "--type",
    ...
    parser.add_argument(
        "-I", "--indent", type=int, default=40, help="缩进树的缩进量.(默认为 40)"
    )
...
def render(input, output="TreeMap.html", direction="LR", indent=40, type=None):
    """
    渲染数据生成网页
